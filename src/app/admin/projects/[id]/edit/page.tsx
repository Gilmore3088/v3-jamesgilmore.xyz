...
export default function EditProjectPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!formData.get("is_friend_project")) {
      formData.set("is_friend_project", "false");
    }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-3">
        <Link href="/admin/projects" className="text-sm text-text-muted transition-colors hover:text-gold">Projects</Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border border-border bg-surface p-6">
        ...
        <div>
          ...
          <textarea id="content" name="content" rows={12} defaultValue={project.content ?? ""}
            className="w-full rounded-md border border-border bg-surface-light px-3 py-2 text-sm text-text-primary outline-none transition-colors focus:border-gold font-mono" />
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <div>
            ...
            <input id="technologies" name="technologies" type="text" defaultValue={(project.technologies ?? []).join(", ")}
              className="w-full rounded-md border border-border bg-surface-light px-3 py-2 text-sm text-text-primary outline-none transition-colors focus:border-gold" />
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <div>
            <label htmlFor="github_url" className="mb-1 block text-sm font-medium text-text-secondary">GitHub URL</label>
            <input id="github_url" name="github_url" type="url" defaultValue={project.github_url ?? ""}
              className="w-full rounded-md border border-border bg-surface-light px-3 py-2 text-sm text-text-primary outline-none transition-colors focus:border-gold" />
          </div>
          <div>
            <label htmlFor="project_url" className="mb-1 block text-sm font-medium text-text-secondary">Project URL</label>
            <input id="project_url" name="project_url" type="url" defaultValue={project.project_url ?? ""}
              className="w-full rounded-md border border-border bg-surface-light px-3 py-2 text-sm text-text-primary outline-none transition-colors focus:border-gold" />
          </div>
        </div>
        ...
        <div>
          ...
          <input id="thumbnail_url" name="thumbnail_url" type="url" defaultValue={project.thumbnail_url ?? ""}
            className="w-full rounded-md border border-border bg-surface-light px-3 py-2 text-sm text-text-primary outline-none transition-colors focus:border-gold" />
        ...
        <div className="grid gap-6 sm:grid-cols-4">
          ...
          <div className="flex items-center gap-3 pt-6">
            <input id="featured" name="featured" type="checkbox" value="true" defaultChecked={project.featured}
          ...
          <div className="flex items-center gap-3 pt-6">
            <input id="is_friend_project" name="is_friend_project" type="checkbox" value="true" defaultChecked={project.is_friend_project}
              className="h-4 w-4 rounded border-border bg-surface-light accent-gold" />
            <label htmlFor="is_friend_project" className="text-sm text-text-secondary">Friend&apos;s project</label>
          </div>
        </div>
